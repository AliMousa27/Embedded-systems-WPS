...
// sets bits for gear depending on number
byte get_gear(byte current, int gear);
// sets bits if key is open (1 or 0)
byte is_key_open(byte current, int key);
...
int main(int argc, char *argv[]) {
    ...
    for(int i=1; i<argc; i++){
        // convert arg to int
        int num = atoi(argv[i]);
        // check if arg is not 0 or 1 or 2 and if its not the gear arg as all other args should be 0 or 1 but the gear
        if((num < 0 || num > 2) && i != 2){
            printf("the arg that caused the error is %s", argv[i]);
            printf("invalid");
            // exit program
            return 0;
        }
        // check if gear arg is not between 0 and 7 and i is 2 as gear arg is the 3rd arg
    ...
    int curr_gear = atoi(argv[2]);
    int key = atoi(argv[3]);
    int b1 = atoi(argv[4]);
    int b2 = atoi(argv[5]);
    // call functions to set bits
    bits = is_engine_on(bits,engine);
    bits = get_gear(bits,curr_gear);
    bits = is_key_open(bits,key);
    bits = break1(bits,b1);
    bits = break2(bits,b2);
...
byte get_gear(byte current, int gear){
    //create array of bytes with the bits in the right position for each gear
    byte bits_position[8] = { 0b00000000, 0b00010000, 0b00100000, 0b00110000, 0b01000000, 
    0b01010000, 0b01100000, 0b01110000 };
    //mask the current with the index of the gear
    return current | bits_position[gear];
}

byte is_key_open(byte current, int key){
    //shift the key bit to the left 2 times and mask the current with it
    return current | key << 2;
}
